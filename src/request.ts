 ...
 * @returns
 */
export const request = async <T>(url: string, options?: IRequestOptions): Promise<T | void> => {
  const defaultOptions: IRequestOptions = {
    ...
    toastHandler: (msg) => console.error('您还没有配置toastHandler，请根据您的UI组件库配置合适的提示方法。'),
  };
  options = Object.assign({}, defaultOptions, options || {});
  const { params, data, json, method, headers, baseUrl, fetchOptions, checkSuccess,
    afterRequest, errorHandler, errorMessage, responseConverter } = options || {};
  if (params) {
  ...
  let resp = null;
  try {
    resp = await fetch(`${baseUrl || ''}${url}`, fetchOptions).then((resp) => resp.json());
  } catch (e: any) {
    console.error(e);
 ...
 */
export const Request: RequestConstructor = function (this: typeof request, overrideDefaultOptions: IRequestOptions) {
